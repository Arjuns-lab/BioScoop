...
import { User, Download as DownloadIcon, Palette, Settings as SettingsIcon, Shield, Trash2, Check, LayoutDashboard, ChevronRight } from 'lucide-react';
...
const SettingsPage: React.FC<SettingsProps> = ({ user, logout }) => {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    // Ideally trigger a re-fetch of user in App.tsx or updated local state, 
    // but dataService updates localStorage so a refresh works or if App checks user state
  };
                      ...
                      <form onSubmit={handleUpdateProfile} className="max-w-md space-y-4">
                         ...
                         <div>
                            ...
                            <input 
                               ...
                               value={user.email}
                               disabled
                               className="w-full bg-dark-950 border border-gray-700 rounded-lg p-3 text-gray-500 cursor-not-allowed"
                            />
                ...
                {activeSection === 'downloads' && (
                   <div className="animate-in fade-in slide-in-from-right-4">
                      <h2 className="text-xl font-bold text-white mb-6 border-b border-gray-800 pb-4 flex justify-between items-center">
                          ...
                          <span className="text-xs font-normal text-gray-400 bg-gray-800 px-2 py-1 rounded">Local Storage</span>
                      </h2>
                                  ...
                                  <div className="relative aspect-video">
                                     ...
                                     <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                         <Link to={`/watch/${item.contentId}`} className="bg-white text-black p-2 rounded-full hover:scale-110 transition-transform">
                                            <SettingsIcon size={20} className="hidden" /> {/* Dummy icon */}
